...
.SYNOPSIS
    ...
    Each task fires once at kick-off + 3h (local time) and runs the one-shot:
        ...
        python -m laliga.run_match --fotmob-id <ID> --season <SEASON>
...
.DESCRIPTION
    Reads laliga/schedules/SCHEDULE_<Season>.json (from build_schedule.py). Tasks live in
    the Task Scheduler folder "\LaLiga\". StartWhenAvailable means a run missed because the
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File laliga\register_tasks.ps1 -Season 2025-26
    powershell -ExecutionPolicy Bypass -File laliga\register_tasks.ps1 -Season 2026-27 -DaysAhead 14
#>
...
Write-Host ""
Write-Host "La Liga Task Registration - season $Season" -ForegroundColor Cyan
Write-Host ("=" * 70)
...
foreach ($g in $games) {
    ...
    $fid   = $g.fotmob_id
    $taskName = "LaLiga_" + $Season.Replace('-','') + "_" + $fid + "_" + $hName + "_vs_" + $aName

    $pyArgs = "-m laliga.run_match --fotmob-id " + $fid + " --season " + $Season
    ...
    Register-ScheduledTask -TaskName $taskName -TaskPath $TaskFolder -Action $action -Trigger $trigger -Settings $settings -Description ("La Liga auto scrape+render+push " + $g.home + " vs " + $g.away) -Force | Out-Null
...
Write-Host ""
Write-Host "View tasks:   Get-ScheduledTask -TaskPath '\LaLiga\*'"
Write-Host "Remove all:   powershell -ExecutionPolicy Bypass -File laliga\unregister_tasks.ps1"
Write-Host ""
