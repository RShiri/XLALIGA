...
log = logging.getLogger("laliga.scraper")
logging.basicConfig(
    level=logging.INFO,
...
POLL_INTERVAL = int(os.environ.get("LALIGA_SCRAPE_POLL_SECONDS", 300))  # 5 min

# FotMob La Liga league id (name "LaLiga" in the XML feed; NOT "LaLiga2" id 901075).
...
LALIGA_WS_BASES = [
    u.strip() for u in os.environ.get(
        ...
        os.environ.get("LALIGA_WHOSCORED_URL", _WS_DEFAULT_URLS),
    ).split("|") if u.strip()
]
LALIGA_WS_BASE = LALIGA_WS_BASES[0]  # backward-compat alias

_fetched_ids: set[int] = set()  # avoid re-fetching in the same run

# La Liga clubs keep stable names across FotMob/WhoScored/Understat, so no
# placeholder-name remapping is needed (unlike the WC play-off slots). Kept as an
# empty hook in case a feed ever uses an odd alias.
...
def fotmob_fetch_wc_matches(dates: "list[str] | None" = None) -> list[dict]:
    ...
    for date_str in dates_to_check:
        ...
        for league in root.iter("league"):
            ...
            # Filter to La Liga: the top flight is name "LaLiga" id 87 in this feed
            # (the second tier is "LaLiga2" id 901075, which we must exclude).
            if str(league.get("id", "")) != str(LALIGA_FOTMOB_ID) \
               and league_name.strip().lower() not in LALIGA_FOTMOB_NAMES:
                continue
...
def sofascore_fetch_match_details(home: str, away: str, around_date: str | None) -> dict:
    ...
    try:
        from laliga.knockout_resolve import _team_key  # alias-aware name compare
    except Exception:
...
def _ev_is_shootout(e: dict) -> bool:
    ...
    Mirrors laliga_dashboard/xg_model.is_shootout (scraper can't import the dashboard
...
def _parse_fotmob_venue(fm_data: dict) -> dict:
    ...
    return {
        ...
        "country": stadium.get("country") or general.get("venueCountry", "Spain"),
        "stage":   stage,
    }
...
def build_match_json(fm_data: dict, ws_data: dict | None,
    ...
    laliga match schema expected by renderer.py.
    ...
    venue_info = _parse_fotmob_venue(fm_data) if not fotmob_unavailable else {
        "venue": "", "city": "", "country": "Spain", "stage": "Group Stage"
    }
    ...
    return {
        ...
        "meta": {
            ...
            "country":    venue_info.get("country", "Spain"),
            "date":       date_str,
...
def fetch_and_save(fotmob_id: int, season: str = "2025-26", fotmob_only: bool = False,
    ...
    # Understat — La Liga's xG + shot-level source (replaces SofaScore). Attempt it for
    ...
    if os.environ.get("UNDERSTAT_FALLBACK", "1") != "0":
        ...
        try:
            from laliga.understat import understat_fetch_match_details
...
def watch_loop(fotmob_only: bool = False) -> None:
    ...
    log.info("Watcher started. Polling every %ds for LALIGA matches …", POLL_INTERVAL)
    while True:
        try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LALIGA Match Scraper")
