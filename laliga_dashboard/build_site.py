...
"""One command to (re)build the whole web dashboard.
    ...
    py laliga_dashboard/build_site.py            # build everything once
    py laliga_dashboard/build_site.py --watch    # rebuild whenever a match JSON changes
    py laliga_dashboard/build_site.py --serve     # build once, then serve on :8777
    py laliga_dashboard/build_site.py --watch --serve
...
def serve():
    ...
    url = f"http://localhost:{port}/laliga_dashboard/index.html"
