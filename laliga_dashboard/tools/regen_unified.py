...
def main():
    ...
    # --- data.js (update per-match xg + rebuild xgRecords for the xG lab) ---
    ...
    for m in data["seasons"][SEASON].get("matches", []):
        ...
        updated += 1
    # xgRecords power the xG Efficiency Lab (scatter / correlation / G−xG chart);
    # rebuild them from the recalibrated matches, mirroring build_data.build_xg_records.
    recs = []
    for m in data["seasons"][SEASON].get("matches", []):
        if not m.get("played") or m.get("xg_home") is None or m.get("xg_away") is None:
            continue
    ...
    data["seasons"][SEASON]["xgRecords"] = recs
