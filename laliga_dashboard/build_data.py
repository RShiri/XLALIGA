#!/usr/bin/env python3
"""Build data.js for the La Liga dashboard — one payload per season, from the FotMob
...
league is driven by its **schedule**: ``laliga/schedules/SCHEDULE_<season>.json`` already
carries every fixture with real scores + matchday (built token-free from FotMob by
``laliga/build_schedule.py``). Rich per-match data (xG, shots, events) is *layered on*
when present in ``laliga/matches/<season>/<fotmob_id>.json`` — the same files the browser
scrapers (``laliga/run_match.py`` / ``laliga/backfill.py``) produce. Matches with no rich
...
CREST_DIR = os.path.join(ROOT, "team_logos", "laliga")
...
def _crest(team, team_id):
    ...
    if os.path.exists(local):
        return f"../team_logos/laliga/{team}.png"
...
def main():
    ...
    for season in ("2024-25", "2025-26", "2026-27"):
        ...
        if s is not None:
            seasons[season] = s
    if not seasons:
        raise SystemExit("No schedules found in " + SCHED_DIR + " — run laliga/build_schedule.py first.")
